                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer" id="data_table">
                        ...
                        <thead>
                            ...
                            <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-250px" style="text-align: start;">{{ trans('volunteer-request.name') }}</th>
                                <th class="min-w-100px" style="text-align: start;">{{ trans('volunteer-request.email') }}</th>
                                <th class="min-w-100px" style="text-align: start;">{{ trans('volunteer-request.read_status') }}</th>
...
@section('js')
        ...
        $(document).ready(function() {
            $(function() {
                var table = $('#data_table').DataTable({
                    ...
                    ajax: "{{ route('volunteer-request.index') }}",
                    columns: [{
                            data: 'id',
                            name: 'id'
                        },
    ...
    <script>
        function DeleteVolunteerRequest(id, element) {
            Swal.fire({
                title: '{{ trans('delete_popup.title') }}',
                text: "{{trans('delete_popup.request')}}",
                icon: 'warning',
        ...
        function performDelete(id, element) {
            axios.delete('{{LaravelLocalization::getCurrentLocale()}}/dashboard/volunteer-request/' + id)
